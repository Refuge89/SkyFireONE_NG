...
void CreatureEventAIMgr::LoadCreatureEventAI_Scripts()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!sCreatureStorage.LookupEntry<CreatureTemplate>(temp.creature_id))
            {
            ...
            switch (temp.event_type)
            {
                ...
                case EVENT_T_SUMMONED_JUST_DESPAWN:
                    if (!sCreatureStorage.LookupEntry<CreatureTemplate>(temp.summon_unit.creatureId))
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; j++)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_MORPH_TO_ENTRY_OR_MODEL:
                        ...
                        {
                            if (action.morph.creatureId && !sCreatureStorage.LookupEntry<CreatureTemplate>(action.morph.creatureId))
                            {
                    ...
                    case ACTION_T_SUMMON:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.summon.creatureId))
                    ...
                    case ACTION_T_CAST_EVENT:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.cast_event.creatureId))
                    ...
                    case ACTION_T_CAST_EVENT_ALL:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.cast_event_all.creatureId))
                    ...
                    case ACTION_T_RANDOM_PHASE_RANGE:       //PhaseMin, PhaseMax
                        ...
                        break;
                    case ACTION_T_SUMMON_ID:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.summon_id.creatureId))
                    ...
                    case ACTION_T_KILLED_MONSTER:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.killed_monster.creatureId))
                    ...
                    case ACTION_T_UPDATE_TEMPLATE:
                        if (!sCreatureStorage.LookupEntry<CreatureTemplate>(action.update_template.creatureId))
            ...
            if (CreatureTemplate const* cInfo = sCreatureStorage.LookupEntry<CreatureTemplate>(temp.creature_id))
            {
                if (!cInfo->AIName || !cInfo->AIName[0])
                {
                    ...
                    const_cast<CreatureTemplate*>(cInfo)->AIName = new char[len];
