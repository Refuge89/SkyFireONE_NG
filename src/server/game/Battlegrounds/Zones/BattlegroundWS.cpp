...
void BattlegroundWS::RespawnFlag(uint32 Team, bool captured)
{
    if (Team == ALLIANCE)
    {
        sLog->outDebug (LOG_FILTER_NETWORKIO, "Respawn Alliance flag");
        m_FlagState[BG_TEAM_ALLIANCE] = BG_WS_FLAG_STATE_ON_BASE;
    }
    else
    {
        sLog->outDebug (LOG_FILTER_NETWORKIO, "Respawn Horde flag");
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    sLog->outDebug (LOG_FILTER_NETWORKIO, "BatteGroundWS: BG objects and spirit guides spawned");
