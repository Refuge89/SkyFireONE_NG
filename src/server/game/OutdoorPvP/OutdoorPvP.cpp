...
{
    sLog->outDebug (LOG_FILTER_NETWORKIO, "Creating capture point %u", entry);
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    if (!m_Creatures[type])
    {
        sLog->outDebug (LOG_FILTER_NETWORKIO, "opvp creature type %u was already deleted", type);
        return false;
    }
    ...
    sLog->outDebug (LOG_FILTER_NETWORKIO, "deleting opvp creature type %u", type);
...
void OutdoorPvP::HandlePlayerLeaveZone(Player * plr, uint32 /*zone*/)
{
    ...
    sLog->outDebug (LOG_FILTER_NETWORKIO, "Player %s left an outdoorpvp zone", plr->GetName());
}
...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    Skyfire::PlayerListSearcher<Skyfire::AnyPlayerInObjectRangeCheck> searcher(players, checker);
