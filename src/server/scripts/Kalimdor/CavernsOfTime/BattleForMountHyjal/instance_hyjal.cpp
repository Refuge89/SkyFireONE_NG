...
struct instance_mount_hyjal : public ScriptedInstance
{
    ...
    void SetData(uint32 type, uint32 data)
    {
        switch (type)
        {
         ...
         sLog->outDebug (LOG_FILTER_NETWORKIO, "TSCR: Instance Hyjal: Instance data updated for event %u (Data=%u)",type, data);
    ...
    void UpdateWorldState(uint32 id, uint32 state)
    {
        ...
        } else sLog->outDebug (LOG_FILTER_NETWORKIO, "TSCR: Instance Hyjal: UpdateWorldState, but PlayerList is empty!");
    }
