...
struct instance_steam_vault : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        sLog->outDebug (LOG_FILTER_NETWORKIO, "TSCR: Instance Steamvault: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            sLog->outDebug (LOG_FILTER_NETWORKIO, "TSCR: Instance Steamvault: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }
        ...
        switch (type)
        {
            case TYPE_HYDROMANCER_THESPIA:
                if (data == SPECIAL)
                {
                    ...
                    sLog->outDebug (LOG_FILTER_NETWORKIO, "TSCR: Instance Steamvault: Access panel used.");
                }
                Encounter[0] = data;
                break;
